...
export default function Login(){
  return <>
   <div className='min-h-screen pt-6 w-full  max-w-[100%]  mt-0  overflow-hidden  py-12'>
   ...
   <h1 className=' mx-auto title-color max-w-max title'>Login</h1>
    ...
    <form className="container mx-auto w-[80%] md:max-w-[10%] my-1 ">
        <div className='grid grid-cols-6   h-max  md:px-9  gap-6 items-start'>
          ...
          <div className=' col-span-6 h-max  flex flex-col gap-y-[19px]  md:px-[90px] mx-4'>
           
            <div className='flex flex-col gap-y-2 '>
            <label htmlFor="email">Email or user name</label>
            <input type="email" name="email" id="email" className='h-[40.647px] px-2 md:px-[41px]'  placeholder='Enter your Email or username'/>
            </div>
            <div className='flex flex-col gap-y-2 '>
            <label htmlFor="phone">Password</label>
            <input type="password" name="phone" id="phone" className='h-[40.647px] px-2 md:px-[41px]'  placeholder='Enter your password'/>
            </div>
          </div>
        </div>
        <div className='flex flex-col md:flex-row justify-center my-3 w-full my-[15px] gap-4  md:gap-5 items-center py-2 mx-auto'>
          <button type='submit ' className='p-3 w-[50%] md:w-[20%] font-bold bg-[#1E88E5] rounded-md text-2xl text-white shadow-xl shadow-black/20'>Login</button>
          <div>
